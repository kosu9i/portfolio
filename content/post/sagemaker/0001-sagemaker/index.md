...
* ~~インスタンスのスケールアップ, スケールダウンも自動でやってくれる。~~
  ~~（どういうトリガでされるかは不明）~~  
  **最近ドキュメントから記載が削除されていた...手動でインスタンスタイプを変更する必要がありそう。**
...
* SageMaker Studio notebooksの費用がよくわからない。
    ...
    + ~~自動でスケールする、とあるがどういうトリガでスケールアップ、スケールダウンするのかがよく分からない。~~  
      [最新版のドキュメント](https://docs.aws.amazon.com/ja_jp/sagemaker/latest/dg/notebooks-run-and-manage-switch-instance-type.html)を見るに、どうやら結局手動でインスタンスタイプを変更しないとダメっぽい。。
